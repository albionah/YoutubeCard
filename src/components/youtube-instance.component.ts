import {css, customElement, html, LitElement, property, query, state, TemplateResult} from 'lit-element';
...
export class YoutubeInstanceComponent extends LitElement {
	private static overflow: boolean = true;
	...
	protected render(): TemplateResult {
		...
		return html`
			<youtube-video-selector-dialog id="browse_dialog" .youtubeControllerService="${this.youtubeControllerService}">
			...
			<div>
				<div class="media_searching">
					<auto-complete-dialog .youtubeControllerService="${this.youtubeControllerService}">
						<input type="text" placeholder="Hledej video" slot="inputContainer" @keypress=${(event)=>
						this.onSearchInputKeyPressed(event)} @change="${(event) => this.searchQuery = event.target.value}" />
	...
	private renderMediaInfo(youtubeVideo: YoutubeVideo): TemplateResult {
		return html`
			<div class="entity__info__media" ?short=${this.config?.info==='short' } ?short-scroll=${this.config?.info==='scroll' }
				?scroll=${YoutubeInstanceComponent.overflow} style="animation-duration: 1s;">
				<div class="marquee">
	...
	private renderMediaControls(): TemplateResult {
		return html`
			<div class="flex mmp-media-controls__media">
				<ha-icon-button @click=${(event)=> this.watchPrevious(event)} .icon=${ICON.PREV}
					.disabled=${!this.youtubeVideo}>
				</ha-icon-button>
			
				${this.renderPlayOrPauseButton()}
			
				<ha-icon-button @click=${(event)=> this.watchNext(event)} .icon=${ICON.NEXT}
					.disabled=${!this.youtubeVideo}>
				</ha-icon-button>
			</div>
		`;
	...
	private renderPlayOrPauseButton(): TemplateResult {
		return html`
			<ha-icon-button @click=${(event)=> this.playOrPause(event)}
